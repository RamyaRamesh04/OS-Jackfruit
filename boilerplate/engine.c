 ...
 *
 * KEY BUG FIX in this version:
 *   The previous version called free(stack) and free(cfg) immediately after
 *   clone() returned in the parent.  clone() without CLONE_VM behaves like
 *   fork(): the child gets a COW copy of the address space, BUT the provided
 *   clone-stack memory is shared until the child touches it.  More critically,
 *   free(cfg) causes glibc to write heap-metadata into the page that cfg occupies;
 *   on some kernel/glibc combinations this write races with the child reading
 *   cfg->rootfs, cfg->argv_blob, etc., corrupting those fields before execv().
 *   A corrupted rootfs path causes chroot() to fail silently (stderr is already
 *   redirected to the pipe) and the child exits with code 1 immediately --
 *   but the container shows exit=0 because waitpid() picks up the 0 from the
 *   PID-namespace init process, not the child directly.
 *
 *   FIX: use a small pipe ("ready pipe") so the child signals the parent
 *   after it has copied everything it needs from cfg into local variables on the
 *   clone stack.  The parent blocks on read() of the ready pipe before calling
 *   free(stack) / free(cfg).  This guarantees no race.
 *
 * Other fixes retained from previous version:
 *   - Rootfs uniqueness enforcement
 *   - run() SIGINT/SIGTERM forwarding
 *   - Joinable producer threads
 *   - CONTAINER_HARD_LIMIT_KILLED distinct state
 *   - MAX_CONTAINERS enforcement
 *   - Rootfs path validation
...
static int child_fn(void *arg)
{
    ...
    /* ---- Step 7: apply nice value ---- */
    if (local_nice != 0) {
        errno = 0;
        if (nice(local_nice) == -1 && errno)
            ; /* non-fatal */
    }
